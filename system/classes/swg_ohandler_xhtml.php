...
------------------------------------------------------------------------- */
		...
		{
			if (($f_mobile_check)&&($direct_classes['basic_functions']->include_file ($direct_settings['path_themes']."/{$direct_settings['theme']}_mobile/swg_theme.php",2))) { $this->theme = $direct_settings['theme']."_mobile"; }
			elseif ($direct_classes['basic_functions']->include_file ($direct_settings['path_themes']."/{$direct_settings['theme']}/swg_theme.php",2)) { $this->theme = $direct_settings['theme']; }
		}

		if (!isset ($this->theme))
		{
			if (($f_mobile_check)&&($direct_classes['basic_functions']->include_file ($direct_settings['path_themes']."/{$direct_settings['swg_theme']}_mobile/swg_theme.php",2))) { $this->theme = $direct_settings['swg_theme']."_mobile"; }
			else
			{
				$direct_classes['basic_functions']->include_file ($direct_settings['path_themes']."/{$direct_settings['swg_theme']}/swg_theme.php",2);
				$this->theme = $direct_settings['swg_theme'];
			}
		}

		$direct_classes['basic_functions']->settings_get ($direct_settings['path_themes']."/{$this->theme}/swg_theme_up.xml");
...
*/
	/*#ifndef(PHP4) */public /* #*/function output_send_error ($f_type,$f_error,$f_extra_data = "")
	{
		...
		if ((isset ($direct_classes['kernel']))&&(direct_class_function_check ($direct_classes['kernel'],"service_init")))
		{
			$f_service_error = $direct_classes['kernel']->service_init ();
		...
		else { parent::output_send_error ("fatal",$f_error,$f_extra_data."<br /><br />Request terminated"); }
	...
	/*#ifndef(PHP4) */public /* #*/function theme_subtype ($f_subtype)
	{
		...
		if (!$direct_settings['swg_force_notheme'])
		{
			...
			if (!$f_return)
			{
				...
				elseif ((isset ($direct_settings['swg_theme_supported_subtypes']))&&(isset ($direct_settings['swg_theme_supported_subtypes'][$f_subtype]))) { $f_return = $direct_classes['basic_functions']->include_file (($direct_classes['basic_functions']->varfilter ($direct_settings['swg_theme_supported_subtypes'][$f_subtype],"settings")),2); }
			}

			if ($f_return) { $this->theme_subtype = $f_subtype; }
		}
...
$direct_classes['@names']['output'] = "direct_oxhtml";
