...
NOTE_END //n*/
/**
* developer/swg_build_stage3.php
*
...
echo ("
----------------------------------------------------------------------------
(C) $direct_settings[swg_title_txt] (Stage 3 Module Builder)
All rights reserved
----------------------------------------------------------------------------
This tool should be used by sWG module developers only. If you are not a
developer for modules, please download an appropriate sWG package.
		...
		if ($g_file_data)
		{
			...
			if (is_array ($g_xml_node_array))
			{
				...
				if (isset ($g_xml_node_array['xml.item']/*#ifndef(PHP4) */,/* #*//*#ifdef(PHP4):) && isset (:#*/$g_xml_node_array['xml.item']['attributes']['source']/*#ifndef(PHP4) */,/* #*//*#ifdef(PHP4):) && isset (:#*/$g_xml_node_array['xml.item']['attributes']['swgversion']/*#ifndef(PHP4) */,/* #*//*#ifdef(PHP4):) && isset (:#*/$g_xml_node_array['xml.item']['attributes']['pdescriptor']/*#ifndef(PHP4) */,/* #*//*#ifdef(PHP4):) && isset (:#*/$g_xml_node_array['xml.item']['attributes']['pname']))
				{
					...
					$g_pdescriptor = $g_xml_node_array['xml.item']['attributes']['pdescriptor'];
					$g_pname = $g_xml_node_array['xml.item']['attributes']['pname'];
...
------------------------------------------------------------------------- */
	...
	$g_builder_object = ((($g_continue_check)&&($g_pdescriptor)&&($g_pname)&&($g_source_path)) ? new directBuilder () : NULL);
...
------------------------------------------------------------------------- */
	...
	{
		echo "\n> Generating \"$g_pdescriptor\" for ".$g_pname;
...
------------------------------------------------------------------------- */
		...
		$g_builder_object->setExcludeDirs ("_developer,_extras,nbproject");
		$g_builder_object->setExcludeFiles ("INSTALL,CHANGELOG,NEWS,FAQ,LICENSE,build.xml");
		$g_builder_object->setStripPrefix ($g_source_path);
		$g_builder_object->setTargetMetadata ($g_pdescriptor,$g_pname,3,$g_swg_version,$g_swg_iversion);

		if ($g_builder_object->setInclude ($g_source_path)) { $g_builder_object->makeAll (); }
