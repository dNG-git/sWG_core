...
----------------------------------------------------------------------------
$Id: swg_build_stage1.php,v 1.8 2008/02/24 21:05:42 s4u Exp $
#echo(sWGcoreVersion)#
...
------------------------------------------------------------------------- */
	...
	if (file_exists ($direct_settings['path_data']."/settings/swg_build_targets{$g_profile}.php"))
	{
		...
		if ($g_file_data)
		{
			...
			{
				if (isset ($g_xml_node_array['swg_build_file_v1']['stage1']['xml.item']['attributes']['swgversion']))
				{
...
------------------------------------------------------------------------- */
	...
	if ($g_continue_check) { $g_builder_object = new direct_developer_builder (); }
