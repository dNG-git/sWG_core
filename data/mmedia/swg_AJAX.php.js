...
"</tr></tbody>\n" +
"</table></div></div>").on("ajaxStart",djs_swgAJAX_event_show).on("ajaxStop",djs_swgAJAX_event_hide_soon).on ("click",djs_swgAJAX_event_hide)
);
...
function djs_swgAJAX_insert_after (f_params)
{
	if (("id" in f_params)&&(("url" in f_params)||("url0" in f_params)))
	{
		...
		djs_swgAJAX(f_params).done (function (f_data,f_status,f_jqXHR)
		{
			if ((f_data !== null)&&("getElementsByTagName" in f_data)&&(f_data.getElementsByTagName("swgAJAX").length > 0))
			{
				var f_ajax_values = djs_swgAJAX_parse_reply (f_params,f_data),f_content = f_data.getElementsByTagName ("content"),f_javascript = f_data.getElementsByTagName ("javascript");

				if (f_javascript.length == 1) { djs_swgDOM_insert_after ({ data:f_content[0].firstChild.nodeValue,hide:f_ajax_values.content_hide,id:f_params.id,id_inserted:f_params.id_inserted,onInsert:{ func:"djs_swgAJAX_insert_javascript",params:{ data:f_javascript[0].firstChild,id:f_params.id,onInserted:f_params.onInsert,perm_params:{ modal:f_ajax_values.window_modal,title:f_ajax_values.title,width:f_ajax_values.width,window_closeable:f_ajax_values.window_closeable } } },onInserted:f_params.onInserted,perm_params:{ modal:f_ajax_values.window_modal,title:f_ajax_values.title,width:f_ajax_values.width,window_closeable:f_ajax_values.window_closeable } }); }
		...
		}).fail (function (f_jqXHR,f_status)
		{
			...
			{
				var f_ajax_values = djs_swgAJAX_parse_reply (f_params,f_jqXHR.responseXML),f_content = f_jqXHR.responseXML.getElementsByTagName ("content"),f_javascript = f_jqXHR.responseXML.getElementsByTagName ("javascript");
...
function djs_swgAJAX_insert_javascript (f_params)
{
	...
	{
		jQuery("#" + f_params.id).append (jQuery("<script type='text/javascript'></script>").append (f_params.data));
...
{
	var f_return = { content_hide:false,title:null,width:-1,width_max:-1,window_closeable:true,window_modal:true },f_content_hide = f_data.getElementsByTagName ("content_hide"),f_jquery_object = jQuery("#" + f_params.id),f_title = f_data.getElementsByTagName ("title"),
	    f_width = f_data.getElementsByTagName ("width"),f_window_closeable = f_data.getElementsByTagName ("window_closeable"),f_window_modal = f_data.getElementsByTagName ("window_modal");
...
{
	var f_ajax_values = djs_swgAJAX_parse_reply (f_params,f_params.data),f_content = f_params.data.getElementsByTagName ("content"),f_javascript = f_params.data.getElementsByTagName ("javascript");

	if (f_content.length == 1)
	{
		if (f_javascript.length == 1) { djs_swgDOM_replace ({ data:f_content[0].firstChild.nodeValue,hide:f_ajax_values.content_hide,id:f_params.id,id_replaced:f_params.id_replaced,onReplace:{ func:"djs_swgAJAX_insert_javascript",params:{ data:f_javascript[0].firstChild,id:f_params.id,onInserted:f_params.onReplace,perm_params:{ modal:f_ajax_values.window_modal,title:f_ajax_values.title,width:f_ajax_values.width,window_closeable:f_ajax_values.window_closeable } } },onReplaced:f_params.onReplaced,perm_params:{ modal:f_ajax_values.window_modal,title:f_ajax_values.title,width:f_ajax_values.width,window_closeable:f_ajax_values.window_closeable } }); }
